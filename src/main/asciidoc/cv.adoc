...
Я реализовал облегченный брокер объектных запросов для Windows, который обеспечил межпроцессное взаимодействие между процессами на персональном компьютере и с удаленными процессами, запущенными на серверах Unix. Брокер объектных запросов предшествовал общей архитектуре посредника объектных запросов (CORBA) и был основан на усовершенствованной архитектуре сетевых систем (ANSA).
...
* Developed the inter-process communicatio n for co-located Windows applications using Dynamic Data Exchange (DDE).
